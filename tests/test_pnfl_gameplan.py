...
def test_save_persists_despite_violations(
    ...
    # And each violation surfaces as a logger.warning.
    warning_messages = "\n".join(r.message for r in caplog.records if r.levelno == logging.WARNING)
    assert "PSL" in warning_messages
...
def test_save_gameplan_wide_warning_has_no_prefix(
    ...
    duplicate_records = [r for r in caplog.records if r.levelno == logging.WARNING and "Duplicate" in r.message]
