...
- `save` — emits per-violation `logger.warning(...)` calls, writes via `fbpro98-gameplan`, and returns the violation tuple. PNFL violations never block the write — callers that want a strict gate read the returned tuple.
